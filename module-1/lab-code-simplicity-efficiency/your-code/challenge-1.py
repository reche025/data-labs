...
def get_inputs():
    ...
    while a not in word_num.keys() or c not in word_num.keys() or b not in ['minus', 'plus']:
        print('I am not able to answer this question. Check your input.')
        get_inputs()

word_num = {'zero' : 0 ,'one' : 1, 'two' : 2, 'three' : 3, 'four' : 4, 'five' : 5, 'six' : 6, 'seven' : 7, 'eight' : 8, 'nine' : 9, 'ten' : 10} 
num_word = {word_num[key]: key for key in word_num.keys()}

def calculator(num1, num2):
    if b == 'minus':
        if word_num[num1] - word_num[num2] > 0:
            print(num_word[word_num[num1] - word_num[num2]])
        elif word_num[num1] - word_num[num2] < 0:
            print('negative' , num_word[abs(word_num[num1] - word_num[num2])])
    elif b == 'plus':
        print(num_word[word_num[num1] + word_num[num2]])
